...
//for loop that checks if the value is a certain value at a desired index to properly advance the dfa	
  for (int i = 0; i < testString.length(); i++)  {
   ...
   //makes comments green
    if (state==q32){
    	testString= new StringBuilder(testString).insert(i-1, "<font color='green'>").toString();
    }
      
  //makes variables purple
    if (state==q31||state==q19){
    	testString= new StringBuilder(testString).insert(i-1, "<font color='purple'>").toString();
    	i=i+21;
    }
    
    if (state==q20 && (testString.charAt(i-2)=='p' ||testString.charAt(i-2)=='v' )){
    	System.out.println("yep");
    	testString= new StringBuilder(testString).insert(i-2, "<font color='purple'>").toString();
    	i=i+21;
    }
    if (state==q2){
    	testString= new StringBuilder(testString).insert(i-2, "<font color='blue'>").toString();
    	i=i+19;
    }
    if (state==q12|| state==q24){
    	testString= new StringBuilder(testString).insert(i-1, "<font color='orange'>").toString();
    	i=i+21;
    }
  //stops making letters colors
    if (state==q27||state==q20|| state==q35|| state==q6 || state==q15){
    	testString= new StringBuilder(testString).insert(i, "</font>").toString();
    	System.out.print(i);
    	i=i+7;
    }
    
	if (state == Error && errorFree ){
		testString= new StringBuilder(testString).insert(i-1, "<font color='red'>").toString();
		errorFree = false;
	}
  ...
  if (state==Error){
	  testString= new StringBuilder(testString).insert(testString.length(), "</font>").toString();
  }
  return testString;
